...
write.csv(Unemployment_yearly, "Werkloosheid_per_jaar.csv", row.names = FALSE)
...
Rename_Combined_Df <- Educationlevel_by_year %>% 
  rename(
...
df_yearly_transposed <- Unemployment_yearly %>%
  pivot_longer(
    cols = -Jaar
  )
